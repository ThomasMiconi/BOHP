...
dirz = glob.glob('trial-logistic-withz-ETA-0.003-MAXDW-0.01-WPEN-1e-4-UPDATETYPE-RMSPROP-POSWEIGHTS-0-STIMCORR-UNCORRELATED-YSIZE-2-LEARNPERIOD-20-ALPHATRACE-.95')
...
for (num, droot) in enumerate(dirz):
    ...
    plt.fill_between(xx, tq25, tq75, color='blue', alpha=0.3)
    plt.plot(xx, tmedian)
    plt.axis([0, tmean.size*10, 0, .5])
    plt.xlabel('Episode #')
    plt.ylabel('Error')
